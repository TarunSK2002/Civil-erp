...
router.get('/', async (req, res) => {
    ...
    try {
        const payments = await Payment.findAll({
            where,
            include: [
                { model: Site, as: 'Site', attributes: ['SiteName'], required: false },
                { model: Labour, as: 'Labour', attributes: ['Name'], required: false },
                { model: Material, as: 'Material', attributes: ['Name'], required: false },
                { model: Payee, as: 'Payee', attributes: ['Name'], required: false },
                { 
                    model: WeeklyPaySheetItem, 
                    as: 'SheetItem', 
                    required: false,
                    include: [{ model: Payee, as: 'Payee', attributes: ['Name'], required: false }] 
                }
            ],
